          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr,index) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0"  v-for="(spuSaleValue,index) in spuSaleAttr.spuSaleAttrValueList" @click="changeActive(spuSaleAttr,spuSaleValue)" :class="{active:spuSaleValue.isChecked==1}" :key="spuSaleValue.id">
                  {{ spuSaleValue.saleAttrValueName }}</dd>
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- readonly禁止用户输入 -->
                <input autocomplete="off" class="itxt" v-model="skuNum" readonly="readonly" @change="changeSkuNum">
  ...
  export default {
    ...
    methods:{
      ...
      async add(){
        ...
        try {
          let result=await this.$store.dispatch('getAddorUpdate',
          { 
            skuId:this.$route.params.skuId,
            skuNum:this.skuNum
          })
          //存储：本地存储(持久化，关闭应用依然保存，但上限5M)||对话存储(非持久化，关闭对话框就自动删除)
          //存储一般存字符串，如果存对象不好得到数据
          //在浏览器的application中查看
